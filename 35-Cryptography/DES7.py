...
# Helper tables for DES (as before)
IP = [
...
# Function to perform permutation based on a table
...
def des_decrypt_block(block, keys):
    ...
    for i in range(16):
        subkey = keys[15 - i]  # reverse the order of subkeys for decryption
        left, right = right, xor(left, feistel(right, subkey))
    
    return permute(right + left, FP)

# DES decryption for multiple blocks
def des_decrypt(ciphertext, keys):
    blocks = [ciphertext[i:i + 64] for i in range(0, len(ciphertext), 64)]
    decrypted_blocks = [des_decrypt_block(block, keys) for block in blocks]
    return ''.join(decrypted_blocks)

# DES key schedule (generate the 16 subkeys from the given key)
def key_schedule(key):
    key = ''.join(f'{ord(c):08b}' for c in key)
    C, D = key[:28], key[28:]
    subkeys = []
    PC1 = [
        57, 49, 41, 33, 25, 17, 9, 1,
...
def main():
    # Given ciphertext from previous encryption
    ...
    # Convert the decrypted binary string to ASCII (text)
    decrypted_text = ''.join(chr(int(decrypted[i:i+8], 2)) for i in range(0, len(decrypted), 8))
    
    # Print the decrypted result
    print(f"Ciphertext: {ciphertext}")
    print(f"Decrypted text: {decrypted_text.strip()}")
